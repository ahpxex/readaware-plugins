...
async function wordDetailView(
  ctx: PluginContext,
  w: PluginVocabularyEntry,
): Promise<PluginView> {
  const note = ctx.storage.get<string>(noteKey(w));
  return {
    kind: "blocks",
    title: w.term,
    blocks: [
      { kind: "dictionary", entry: w.entry },
      ...(note ? [{ kind: "markdown" as const, markdown: `**Note:** ${note}` }] : []),
      ...(w.context ? [{ kind: "quote" as const, text: w.context, caption: w.bookTitle }] : []),
      {
        kind: "keyValue",
        rows: [
            ...
            run: () => ({
              view: {
                ...
                onSubmit: async (values) => {
                  ...
                  if (text) ctx.storage.set(noteKey(w), text);
                  else ctx.storage.remove(noteKey(w));
...
function reviewStep(
  ...
  const masked = w.context
    ? w.context.replace(
        new RegExp(w.term.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi"),
        "____",
      )
    : w.definition;
  const next = (): PluginViewResult => ({ view: reviewStep(ctx, session, index + 1) });
  return {
    kind: "blocks",
...
const plugin: PluginModule = {
  activate(ctx: PluginContext) {
      ...
      run: (input) => ({
        view: {
          ...
          onSubmit: async (values) => {
            ...
            if (!term) return { toast: "Enter a word first" };
            ...
            if (note) ctx.storage.set(`note:${language}:${term.toLowerCase()}`, note);
    ...
    ctx.ai?.registerTool({
      ...
      label: "Save word",
      description:
        "Look up a word with the built-in dictionary and save it to the user's vocabulary notebook. Include the sentence it appeared in when available.",
      parameters: {
        type: "object",
        properties: {
    ...
    ctx.ai?.registerTool({
      ...
      label: "List words",
      description:
        "List the user's vocabulary notebook entries, newest first. Useful for quizzes or progress questions.",
      parameters: {
        type: "object",
